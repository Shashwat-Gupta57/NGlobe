...
class ProxyConfig(BaseModel):
    ...
    cert_dir: str = str(Path.home() / ".nglobe" / "certs")
...
class GeoIPConfig(BaseModel):
    city_db_path: str = str(Path.home() / ".nglobe" / "geoip" / "GeoLite2-City.mmdb")
    asn_db_path: str = str(Path.home() / ".nglobe" / "geoip" / "GeoLite2-ASN.mmdb")
    cache_size: int = 10_000


class DatabaseConfig(BaseModel):
    nglobe: Literal["sqlite", "postgres"] = "sqlite"
    sqlite_path: str = str(Path.home() / ".nglobe" / "data" / "networkglobe.db")
    postgres_url: str = ""
...
class LoggingConfig(BaseModel):
    ...
    file: str = str(Path.home() / ".nglobe" / "logs" / "networkglobe.log")
    max_size_mb: int = 50
    backup_count: int = 3
